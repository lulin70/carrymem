...
| ベンチマーク | スコア | 方法 |
|-------------|--------|------|
| **LongMemEval**（公式） | **53.3%** | 公式データセット + LLM-as-Judge |
| **RuleEngine-Eval**（独自） | **93.3%** | CarryMem独自ベンチマーク |
...
> *LongMemEval：公式oracleデータセット（500Q層化サンプル90Q）、Judge：Claude Sonnet 4（公式：GPT-4o）。[方法とコンプライアンス](../BENCHMARK_STRATEGY_FINAL.md#compliance-status)*
...
## ベンチマーク結果（Phase 1 — 最適化後）

**総合スコア：94.5%（Aグレード Outstanding）** — 4ベンチマーク、20次元

| ベンチマーク | スコア | グレード | 主要指標 |
|-------------|--------|---------|---------|
| **MSC** | **100.0%** | A+ | 90日間100%再現、AIが本当に「あなたを覚えている」 |
| **LongMemEval** | **91.5%** | A | 100%再現精度、100%プライバシー準拠 |
| **MemEval** | **93.1%** | A | 92%分類精度、88%ゼロコスト |
| **RuleEngine-Eval** | **93.3%** | A | ルールエンジンを持つ唯一のシステム（競合：0%） |
...
- **v0.2.0**: PyPI リリース、アイデンティティレイヤー（whoami、profile エクスポート）、490 テスト
