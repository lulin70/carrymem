...
## 2. 当前推进状态 (Snapshot @ 2026-07-17)

| 优先级 | 总数 | 完成 | 进行中 | 待开始 | 完成率 |
|--------|------|------|--------|--------|--------|
| **P0 立即修复** | 5 | 5 | 0 | 0 | **100%** ✅ |
| **P1 高优先级** | 20 | 6 | 1 | 13 | **30%** 🟡 |
| **P2 中优先级** | 18 | 0 | 0 | 18 | **0%** ⬜ |
| **P3 低优先级** | 11 | 0 | 0 | 11 | **0%** ⬜ |
| **总计** | 54 | 11 | 1 | 42 | **20.4%** |
...
| W1 | **TD-010 (SQLiteAdapter 71%→80%)** | Tester | P7→P9 | 🟡 进行中 | 待 commit |
...
|------|-------|------|---------|------|------|
| W2 | TD-007 (跨层私有访问) | Architect | P2→P3→P8 | 无 (TD-006 前置) | 3h |
| W2 | TD-037 (Mixin 隐式协议) | Architect | P2→P3→P8 | TD-007 | 2h |
| W3 | TD-006 (SQLiteAdapter ISP) | Architect | P2→P3→P4→P8→P9 | TD-007, TD-010 | 4h |
| W4 | TD-005 (cmd_doctor F=62 拆分) | Architect+Coder | P2→P3→P8→P9 | TD-010 | 3h |
| W5 | TD-008a (7 个 E 级函数，非 recall_engine) | Coder | P8→P9 | TD-010 | 2h |
| W5 | TD-008b (2 个 E 级函数，recall_engine) | Coder | P8→P9 | TD-006 | 1h |
...
| **P9 测试执行** | TD-010, TD-005, TD-006, TD-008 | 🟡 依赖 P8 |
...
1. **TD-010 推进方式**: 是作为 Batch 2 收尾单独 commit，还是合并到 Batch 3 启动前的准备 commit?
   - 建议: 单独 commit，作为 Batch 2 收尾

2. **P1 Batch 3 vs Batch 4 并行策略**: 是否同时启动 Batch 3 (架构) 和 Batch 4 (DevOps+代码质量)?
   - 建议: 串行推进 (Batch 3 → Batch 4)，避免架构重构期间代码质量改动造成冲突

3. **P2 推进时机**: 是在 P1 全部完成后 (v0.9.0) 启动，还是在 P1 Batch 4 期间就开始 P2 Group D (DevOps 改进)?
   - 建议: P2 Group D (TD-025/026/042/043) 可与 P1 Batch 4 并行，其余 P2 项待 P1 全部完成后启动

4. **版本号规划**:
   - v0.8.1: P1 Batch 2 收尾 (TD-010) + Batch 4 (DevOps+代码质量)
   - v0.9.0: P1 Batch 3 (架构重构) + P2 全部
   - v0.9.1+: P3 渐进改善
   - 建议: 确认此版本号规划

5. **共识确认**: 本路线图 §5 的 7-Role 投票矩阵是否需要用户作为 PM 角色最终签字?
   - 建议: 用户作为最终决策者审核 §5 投票结果，确认后视为全员共识达成
