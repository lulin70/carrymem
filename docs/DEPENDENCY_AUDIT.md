...
1. **cryptography 库版本过旧** - 存在证书验证绕过漏洞（CVE-2026-34073）
...
| 包名 | 当前版本 | 要求版本 | 用途 | 安全状态 |
|------|----------|----------|------|----------|
| **cryptography** | 48.0.0 | >=41.0 | 加密/解密操作 | 🔴 **需升级** |

#### 🔴 cryptography 详细分析 - **高危**

- **用途**: 字段级加密、密钥管理、Fernet 对称加密
- **当前版本**: 48.0.0
- **最新稳定版**: 46.0.6+ （注意：版本号可能有差异，以官方为准）

**已知漏洞**:

| CVE ID | 严重程度 | 影响版本 | 描述 | 修复版本 |
|--------|----------|----------|------|----------|
| **CVE-2026-34073** | 🔴 高危 | < 46.0.6 | X.509 证书验证绕过 - DNS 名称约束未正确应用于 peer name 验证，可能导致中间人攻击 | >= 46.0.6 |
| CVE-2026-26007 | ⚠️ 中危 | < 46.0.5 | 二进制椭圆曲线私钥部分泄露（仅影响罕见曲线） | >= 46.0.5 |

**技术细节**:
- **CWE 分类**: CWE-295 (证书验证不当)
- **CVSS 评分**: 7.8/10 (高危)
- **攻击场景**: 使用通配符证书时可能绕过子域排除约束
- **影响范围**: 仅影响使用 X.509 证书链验证的功能
...
| # | 问题 | 影响 | 建议 | 截止日期 |
|---|------|------|------|----------|
| 1 | cryptography 版本过旧（CVE-2026-34073） | 证书验证绕过，潜在 MITM 攻击 | 升级到 >= 46.0.6 | **立即** |

### 中优先级（计划内处理）

| # | 问题 | 影响 | 建议 | 截止日期 |
|---|------|------|------|----------|
| 2 | PyYAML 可能的不安全使用 | RCE 风险（如果使用了 load() 而非 safe_load()） | 代码审计 + 单元测试验证 | 2 周内 |
| 3 | sentence-transformers 重型依赖 | 安装包过大（~2GB），启动慢 | 评估是否真正需要语义搜索功能 | 下个版本 |
...
# 更新 setup.py 中的最低版本要求
# 在 setup.py 的 extras_require["encryption"] 中修改为：
"cryptography>=46.0.6",
```
...
3. **最小权限原则**
   ```bash
   # 仅安装必要的扩展
   pip install carrymem  # 核心功能（仅 PyYAML）
   pip install carrymem[language]  # + 语言检测
   pip install carrymem[encryption]  # + 加密功能
   # 避免使用：
   # pip install carrymem[full]  # 包含所有重型依赖
   ```
...
| 日期 | 版本 | 审计人 | 发现的问题 | 状态 |
|------|------|--------|------------|------|
| 2026-06-11 | 0.4.0 | Auto Audit | cryptography CVE-2026-34073, PyYAML 使用审查 | 🔄 待处理 |
...
1. **🔴 立即升级 cryptography** - 修复高危漏洞 CVE-2026-34073
