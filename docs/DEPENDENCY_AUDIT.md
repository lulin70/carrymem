...
| 包名 | 当前版本 | 要求版本 | 用途 | 安全状态 |
|------|----------|----------|------|----------|
| **cryptography** | 48.0.0 | >=46.0.6 | 加密/解密操作 | ✅ **已修复 (v0.6.2)** |

#### cryptography 详细分析 - ✅ 已修复

- **用途**: 字段级加密、密钥管理、Fernet 对称加密
- **当前版本**: 48.0.0
- **setup.py 约束**: >=46.0.6 (v0.6.2 升级，原 >=42.0 → >=46.0.6)
- **状态**: CVE-2026-34073 已通过版本约束升级修复
...
| # | 问题 | 影响 | 建议 | 截止日期 | 状态 |
|---|------|------|------|----------|------|
| 1 | cryptography 版本过旧（CVE-2026-34073） | 证书验证绕过，潜在 MITM 攻击 | 升级到 >= 46.0.6 | **立即** | ✅ 已修复 (v0.6.2) |
