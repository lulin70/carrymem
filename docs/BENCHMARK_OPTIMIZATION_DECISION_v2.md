...
### S.6 下一步优化方向（v0.2.1整理评估后，2026-05-23更新）

| 优先级 | 方向 | 投入 | 回报 | 目标版本 |
|--------|------|------|------|----------|
| **P0-A** | PrefEval违反率专项优化(11→5) | 低(1周) | 高(+2-4pp) | v0.2.2 |
| **P0-B** | 状态型/事件型记忆版本链 + context.py拆分 | 中(2-3周) | 高(LME+10pp启发) | v0.2.3 |
| P1-A | Consolidation定时触发 | 低(1周) | 中 | v0.2.4 |
| P1-B | Motive机制(未完成意图) | 中(1-2周) | 中 | v0.2.4 |
| P2 | PrefEval评测标准化 | 低(3天) | 高 | v0.2.4 |
| P3 | carrymem.py拆分+性能优化 | 高(2周) | 中 | v0.2.5 |

**版本链技术方案**：
- 新增 `memory_nature`(state/event) + `version_chain_id` + `version_number` 字段
- 状态型：写入时检查同topic旧记忆→标记superseded，新记忆指向旧记忆（版本链）
- 事件型：不合并，完整保留，衰减更快
- 查询时：状态型只返回最新版本，事件型按时间倒序
- 向后兼容：user_preference/correction→默认推断state，其余→event

**MemOS竞品启发**：
1. 时间感知（状态型vs事件型）——投入产出比最高，LME +10pp
2. Dream离线反思——规则引擎Phase 3-5的Motive机制加速器
3. 异步解耦（L1→L2→L3）——Consolidation定时触发参考
4. 图存储——P3方向，当前阶段不急
