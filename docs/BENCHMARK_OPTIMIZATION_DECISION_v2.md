...
> 版本：v20.0 — **基于4原则更新优化方案：轻量级+差异化+PrefEval专注+持续进步**
> 核心定位：CarryMem 是 AI 的**身份层**——轻量级、可携带、最懂用户
> 护城河逻辑：**用得越久，越懂你；越懂你，越难离开。**

***

## 零、优化四原则（2026-05-23 产品战略确认）

1. **坚持产品定位**：轻量级，可携带，最懂用户。不做重功能叠加。
2. **吸收竞品优点但坚持差异化**：不盲目叠加功能，保持最突出的差异化点（零LLM分类、规则引擎、偏好注入精准度）。
3. **PrefEval方向专注优化**：PrefEval成绩不错，坚持这个方向优化，README展示历次进步。其他Benchmark可撤。
4. **持续进步更新**：有进步就更新README和文档，展示迭代轨迹。
...
| v0.2.1修复前 (200样本) | 0.827 | 4 | 5 | 30 | 共指消解+redaction |
| v0.2.1修复后 (200样本) | 0.855 | 7 | 2 | 22 | 移除记忆查询指令 |
| **v0.2.1优化后 (200样本)** | **0.870** | 11 | 3 | **17** | **QA prompt简化** |
...
### S.6 下一步优化方向（v20.0，基于4原则调整，2026-05-23）

**原则过滤**：去掉图存储(P3，重功能)、Motive机制(P1-B，非差异化核心)，聚焦轻量级+PrefEval+差异化。

| 优先级 | 方向 | 投入 | 回报 | 目标版本 | 原则符合 |
|--------|------|------|------|----------|----------|
| **P0-A** | PrefEval违反率专项优化(11→5) | 低(1周) | 高(+2-4pp) | v0.2.2 | 原则3:PrefEval专注 |
| **P0-B** | 状态型/事件型记忆版本链 | 中(2-3周) | 高(LME+10pp启发) | v0.2.3 | 原则1:轻量级时间感知 |
| **P1-A** | context.py拆分(920行→5子模块) | 中(1-2周) | 中(可维护性) | v0.2.3 | 原则1:轻量级代码 |
| **P1-B** | Consolidation定时触发 | 低(1周) | 中 | v0.2.4 | 原则1:自动维护 |
| **P2** | PrefEval评测标准化 | 低(3天) | 高 | v0.2.4 | 原则3:可复现 |

**已移除**（不符合4原则）：
- ~~Motive机制~~ — 非差异化核心，MemOS也在做，不急
- ~~carrymem.py拆分+性能优化~~ — 可在P1-A中顺带处理，不单独立项
- ~~图存储/联想检索~~ — 重功能，违反原则1轻量级定位

**差异化护城河**（必须持续加强）：
1. **零LLM分类** — 88%记忆无需LLM token，这是Mem0/MemOS做不到的
2. **规则引擎** — 唯一拥有规则引擎的记忆系统
3. **偏好注入精准度** — PrefEval 87.0% > reminder 83.5%，主动注入 > 全量提醒
