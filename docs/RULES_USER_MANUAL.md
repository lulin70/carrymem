...
- `--soft` (optional): Make this a soft rule (AI can override if context requires)
- `--template` (optional): Use a predefined rule template
- `--interactive` (optional): Interactive mode for rule creation
...
### Version History
- **v0.8.0** (Current): Graphify — 3 MCP graph tools (query_graph, shortest_path, get_memory_impact), edge confidence labels (EXTRACTED/INFERRED/AMBIGUOUS), schema migration v100
