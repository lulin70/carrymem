...
| **依赖** | 无 |
| **状态** | 🟡 代码侧+CI 侧全部完成，OIDC token 交换已验证 (2026-07-21) — release.yml 已加 `environment: pypi` + `id-token: write` 权限 + 分支保护注释；**password 行已删除**（因 PyPI API token 在对话渠道暴露 2 次触发强制撤销流程）；**pypi environment deployment_branch_policy 已清除**（null，接受所有 branches/tags，修复 tag 触发的 run head_branch 是 tag name 而非 branch name 导致被拒绝的问题）；**PEP 440 version normalization 修复**（tag `0.9.3-rc1` 与 wheel `0.9.3rc1` 经 `packaging.version.Version` 规范化后比较，修复 run 29832845134 中 Verify version consistency 步骤失败）；**new-main 分支保护已配置**（6 个 status checks: Lint/Tests/Build&Install/Security Scan/Syntax Check/VSCode Extension Tests）。**OIDC token 交换验证成功**（run 29835177978: GitHub 生成 OIDC token → PyPI 验证 token 有效 → 返回 `invalid-publisher` 因未配置 Trusted Publisher，属预期错误）。**2 个手动步骤待用户执行**：(1) 在 https://pypi.org/manage/account/token/ 撤销暴露的 token；(2) 在 https://pypi.org/manage/project/carrymem/publishing/ 配置 Trusted Publisher（repo=`lulin70/carrymem`、workflow=`release.yml`、environment=`pypi`、tag 正则 `^v\d+\.\d+\.\d+(-rc\d+)?$`）— **唯一阻塞项**；配置完成后通知 AI 重新触发 `v0.9.3-rc1` tag 验证端到端发布；验证成功后执行 `gh secret delete PYPI_API_TOKEN -R lulin70/carrymem` |
