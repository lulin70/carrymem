...
| `.github/workflows/ci.yml` | Added `docker-build` job (needs test+build, Buildx+GHA cache, smoke test, image size); fixed YAML syntax in `Show image size` step (block scalar `|`); **TD-062**: simplified smoke test to import-only (was `CarryMem()` instantiation which hung in CI) + `timeout --kill-after=5 120` + `docker run --init` (without `--kill-after`, `timeout` hangs forever because `docker run` does not propagate SIGTERM to the container — confirmed in CI run 30142136016) |
...
| `extensions/vscode-carrymem/test/runVscodeTests.js` | **TD-061**: `extensionDevelopmentPath` from `path.resolve(__dirname, '..')` (resolves to `out/`) to `path.resolve(__dirname, '..', '..')` (resolves to extension root `extensions/vscode-carrymem/`); **TD-061b**: renamed `testsPath` → `extensionTestsPath` in `runTests()` call — `@vscode/test-electron` API requires `extensionTestsPath` (NOT `testsPath`), passing `testsPath` silently set `options.extensionTestsPath` to `undefined` → VSCode received `--extensionTestsPath=undefined` → `Cannot find module '/undefined'` (nightly run 30140373325); **TD-061c**: `extensionTestsPath` now points to `runTests.js` (mocha runner) instead of `extension.test.js` — VSCode's extension host does NOT set up mocha automatically, so pointing to the test file directly produces `ReferenceError: describe is not defined` (nightly run 30143639995) |
| `extensions/vscode-carrymem/test/runTests.js` | **TD-061c**: New mocha test runner for VSCode extension host — exports `run()` function returning a Promise (required by VSCode's `--extensionTestsPath` contract), creates Mocha instance (BDD, 30s timeout), loads `extension.test.js`, resolves/rejects based on test failures |
| `extensions/vscode-carrymem/package.json` | **TD-061c**: `compile` script simplified `cp test/runUnitTests.js out/test/ && cp test/runVscodeTests.js out/test/` → `cp test/*.js out/test/` (future-proof for new runner scripts) |
| `.github/workflows/ci.yml` + `.github/workflows/nightly.yml` | **TD-061c**: Copy step simplified to `cp test/*.js out/test/` (matches package.json compile script) |
