...
| `extensions/vscode-carrymem/test/runVscodeTests.js` | **TD-061**: `extensionDevelopmentPath` from `path.resolve(__dirname, '..')` (resolves to `out/`) to `path.resolve(__dirname, '..', '..')` (resolves to extension root `extensions/vscode-carrymem/`); **TD-061b**: renamed `testsPath` → `extensionTestsPath` in `runTests()` call — `@vscode/test-electron` API requires `extensionTestsPath` (NOT `testsPath`), passing `testsPath` silently set `options.extensionTestsPath` to `undefined` → VSCode received `--extensionTestsPath=undefined` → `Cannot find module '/undefined'` (nightly run 30140373325) |
