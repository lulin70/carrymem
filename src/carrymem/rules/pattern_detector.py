...
class PatternDetector:
    ...
    def detect_patterns(self, memories: List[Dict], memory_type: Optional[str] = None) -> List[MemoryPattern]:
        """
    ...
    def _detect_type_patterns(self, mem_type: str, memories: List[Dict]) -> List[MemoryPattern]:
    ...
    def _detect_preference_patterns(self, memories: List[Dict]) -> List[MemoryPattern]:
        ...
        for kw, matching in keyword_map.items():
            if len(matching) >= self.min_occurrences:
                neg_count = sum(1 for m in matching if self._detect_negation(m.get("content", "")))
        ...
        for domain, matching in domain_map.items():
            ...
            if len(types_in_domain) >= 2 and len(matching) >= self.min_occurrences:
                ...
                for m in matching:
                    all_keywords.extend(self._extract_keywords(m.get("content", "")))
                top_keywords = [kw for kw, _ in Counter(all_keywords).most_common(3)]
                has_negation = any(self._detect_negation(m.get("content", "")) for m in matching)
                confidence = self._calc_confidence(len(matching))
                patterns.append(
                    MemoryPattern(
