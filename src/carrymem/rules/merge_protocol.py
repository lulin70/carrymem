...
def _resolve_incoming_conflicts(
    inc: Rule,
        ...
        if decision == MergeDecision.KEEP_INCOMING:
            ...
            audit_entries.append(
                _make_audit_entry(
                    ...
                    reason=(f"Existing [{conflict.existing_rule.scope}] " f"overrides incoming [{inc.scope}]"),
                )
            )
