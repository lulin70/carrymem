...
class PatternAnalyzer:
        ...
        if language == "en":
            preference_keywords.extend(
                [
                    ...
                    "hate", "dislike", "adore", "default", "standard", "convention",
                    "style", "approach", "choice", "habit", "practice", "routine",
                    "ritual", "rule", "policy", "always", "never", "usually",
                    "typically", "normally", "consistently", "over", "instead of",
                    "rather than", "better than", "worse than", "i love", "i like",
                    "i hate", "i enjoy", "i prefer", "i always", "i never",
            ...
            preference_keywords.extend(
                [
                    "爱好", "喜好", "偏爱", "最爱的", "喜欢的", "偏好", "喜欢", "爱",
                    "讨厌", "不喜欢", "习惯", "通常", "总是", "从不", "默认", "标准",
                    "风格", "选择", "觉得", "认为", "看法", "观点", "意见", "倾向于",
                    "倾向于用", "习惯用", "喜欢用", "别用", "不要用", "避免",
                ]
            )
        elif language == "ja":
            preference_keywords.extend(
                [
                    "好き", "嫌い", "好む", "欲しい", "嫌う", "好きです",
                    "好きではありません", "いいです", "ほしい", "いつも", "常に",
                    "通常", "習慣", "デフォルト", "標準", "スタイル", "選択",
                    "好み", "傾向", "使いたい", "使ってください", "お願いします",
                    "避けて", "使わず",
                ]
            )
    ...
    def _detect_correction_pattern(self, message: str, language: str) -> Optional[Dict[str, Any]]:
        ...
        strong_correction_keywords = [
            ...
            "change our", "change strategy", "different approach", "should be",
            "needs to be", "actually is", "supposed to be",
        ]
    ...
    def _detect_fact_pattern(self, message: str, language: str) -> Optional[Dict[str, Any]]:
        ...
        noise_indicators = [
            "ok", "yeah", "sure", "thanks", "nice", "cool", "great",
        ...
        if language == "en":
            task_keywords.extend(
                [
                    ...
                    "add", "create", "build", "make", "generate", "develop",
                    "write", "update", "upgrade", "migrate", "integrate", "deploy",
                    "release", "plan", "design", "architect", "research",
                    "investigate", "analyze", "review", "test", "validate",
                    "verify", "check", "monitor", "need to", "should", "must",
                    "have to", "require", "going to", "todo", "task",
            ...
            task_keywords.extend(
                [
                    "实现", "重构", "优化", "修复", "调试", "解决", "添加", "创建",
                    "构建", "制作", "生成", "开发", "编写", "更新", "升级", "迁移",
                    "集成", "部署", "发布", "计划", "设计", "研究", "调查", "分析",
                    "审查", "测试", "验证", "检查", "监控", "需要", "应该", "必须",
                    "要", "待办", "下一步", "每次", "每周", "每天", "定期", "例行",
                    "站会", "冒烟测试", "代码审查",
                ]
            )
        elif language == "ja":
            task_keywords.extend(
                [
                    ...
                    "追加", "作成", "構築", "生成", "開発", "記述", "更新",
                    "アップグレード", "移行", "統合", "デプロイ", "リリース",
                    "計画", "設計", "研究", "調査", "分析", "レビュー", "テスト",
                    "検証", "確認", "監視", "必要", "すべき", "必須", "やるべき",
                    "次のステップ", "毎回", "毎週", "毎日", "定期的", "ルーティン",
    ...
    def _detect_decision_pattern(self, message: str, language: str) -> Optional[Dict[str, Any]]:
        ...
        if language == "en":
            ...
            decision_keywords.extend(
                [
                    "决定", "选定", "确定", "采用", "选用", "敲定", "方案", "策略",
                    "架构", "共识", "一致同意", "我们用", "选了", "就用",
                ]
            )
        elif language == "ja":
            decision_keywords.extend(
    ...
    def _detect_sentiment_pattern(self, message: str, language: str) -> Optional[Dict[str, Any]]:
        ...
        if language == "en":
            sentiment_keywords.extend(
                [
                    "love", "hate", "dislike", "enjoy", "happy", "sad", "angry",
                    "excited", "frustrated", "upset", "annoyed", "bored", "tired",
                    "exhausted", "inspired", "proud", "confident", "worried",
                    "scared", "relaxed", "loathe", "detest", "disappoint",
                    "delighted", "thrilled", "depressed", "irritated", "panic",
                    "frighten", "motivated", "too slow", "too fast", "too hard",
                    "too easy", "painful", "annoying", "clunky", "laggy",
                    "awesome", "fantastic", "wonderful", "amazing", "terrible",
                    "horrible", "awful", "brilliant", "superb", "outstanding",
                ]
            )
        elif language.startswith("zh"):
            sentiment_keywords.extend(
                [
                    "棒", "很棒", "真好", "太好了", "优秀", "惊人", "可怕", "糟糕",
                    "恶心", "恐怖", "喜欢", "讨厌", "烦", "崩溃", "心碎", "开心",
                    "难过", "生气", "激动", "不满", "超赞", "无语", "无语了",
                    "太赞了", "绝了", "牛", "牛逼", "累", "疲惫", "无聊", "焦虑",
                    "担心", "害怕", "自豪", "自信", "放松", "失望", "郁闷", "不爽",
                    "太慢", "太烦", "很担心", "棒极了",
                ]
            )
        elif language == "ja":
            sentiment_keywords.extend(
                [
                    "嬉しい", "悲しい", "怒っている", "興奮", "失望", "満足",
                    "素晴らしい", "最高", "ひどい", "最悪", "好き", "嫌い",
                    "楽しい", "面白い", "つまらない", "感動", "驚き", "不安",
                    "心配", "怖い", "イライラ", "ストレス", "疲れた", "うんざり",
                    "すごい", "やばい", "素敵", "残念", "遅い", "不便", "迷惑",
                ]
            )
    ...
    def _detect_location_pattern(self, message: str, language: str) -> Optional[Dict[str, Any]]:
        ...
        for keyword in keywords:
            if keyword in msg_lower:
                common_locations = {
                    "en": [
                        "park", "station", "airport", "hotel", "restaurant",
                        "office", "building", "street", "avenue", "road",
                    ],
                    "zh-cn": [
                        "公园", "车站", "机场", "酒店", "餐厅", "办公室",
                        "大楼", "街道", "大道", "路",
