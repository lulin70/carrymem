...
def register_decision_patterns(registry: PatternRegistry) -> None:
    ...
    b.add_decision_batch(
        [
            ("decide_verb", r"\b(decision|decided?|choose|chose|chosen|choice|select|selected|selection|pick|picked)\b", "strong"),
            ...
            ("finalize", r"\b(final(ize|ized|ization)|confirm(ed|ation)|approve(d|val)?|sign(ed|ing|off)?)\b", "strong"),
            ("arch_is", r"\b(architecture|approach|strategy|plan|pattern|design|solution|stack|framework|library|tool|tech|technology)\s+(is|will be|should be|has been|we(\'re| are))\b", "strong"),
            ("use_instead", r"\b(use|using|utilizing|leveraging)\s+\w+\s+(instead\s+of|over|rather\s+than)\b", "strong"),
            ("use_for", r"^use\s+\w+\s+(for|in|as|to)\s+\w+", "strong"),
            ("we_will_use", r"\b(we|team|group)\s+(\'ll|will|are|\'re)\s+(use|using|adopt|adopting|go\s+with|move\s+to|switch\s+to|build\s+with|deploy\s+(with|on|to))\b", "strong"),
            ("agreed_to", r"\bagreed?\s+to\s+(use|adopt|go\s+with|switch\s+to|implement|build|deploy)\b", "strong"),
            ("decided_to", r"\bdecided?\s+to\s+(use|adopt|go\s+with|switch\s+to|migrate|move|build|deploy|implement)\b", "strong"),
            ("from_now_on", r"\b(from\s+now\s+on|going\s+forward|henceforth)\b", "strong"),
            ("mandatory", r"\b(are|is|will be)\s+(mandatory|required|standard|policy|rule|practice|norm|convention|default)\b", "strong"),
        ],
        flags=re.IGNORECASE,
    )
