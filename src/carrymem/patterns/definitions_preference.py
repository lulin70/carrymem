...
def register_preference_patterns(registry: PatternRegistry) -> None:
    ...
    b.add_preference_batch(
        [
            ("prefer_over", r"\b(i\s+)?(prefer|preference|favor|favour|rather)\b.*"
             r"\b(over|instead of|to|than|rather than)\b", "strong"),
            ("use_over", r"\b(use|using|utilizing|adopting|choosing|picking|going with)\b.*"
             r"\b(over|instead of|rather than|not)\b", "strong"),
            ("always_use", r"^(i\s+)?(always|never|generally|typically|usually|normally|consistently)"
             r"\s+(use|prefer|like|love|hate|avoid|stick to|go for)\b", "strong"),
            ("my_default", r"\b(my\s+)?(default|standard|convention|practice|habit|rule|policy|"
             r"preference|style|choice|approach|way)\s+(is|are|will be|has been|should be)\b",
             "strong"),
            ("when_coding", r"\b(when|while|whenever|if)\s+(writing|coding|developing|building|"
             r"working)\s+\w+,\s*i\s+(always|usually|prefer|like|tend to)\b", "strong"),
            ("context_prefer", r"\b(for|in|on|during)\s+\w+.*(i\s+)?(prefer|use|choose|pick|like|"
             r"stick to|go with)\b", "strong"),
            ("really_like", r"\b(i\s+)?(really\s+)?(like|love|enjoy|appreciate|admire|adore|hate|"
             r"dislike|can\'t stand|despise|loathe)\b", "medium"),
