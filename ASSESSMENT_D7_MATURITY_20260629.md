...
| P1-7 | release.yml 从未触发 | **✅ 已完成** (commits b594146 + 7c9c7e3, tag v0.4.1): 添加 `timeout-minutes: 30` + 版本一致性验证步骤; 配置 `PYPI_API_TOKEN` secret; 创建并推送 v0.4.1 tag 触发首次自动发布。Release workflow 35s 全绿: Build → Check → 版本一致性验证 → Publish to PyPI → GitHub Release。PyPI Latest: 0.4.1, GitHub Release: v0.4.1 (wheel + sdist) |
